'''
Implementation taken from https://colab.research.google.com/drive/18RGz4m4uqtBIk3axaoKKZ5VLfVyzV-Tl#scrollTo=kkqhcedsqB3I
Numerics of GANs repository inspired version
'''
...
class Generator(nn.Module):
    def __init__(self, z_dim, c_dim, gf_dim, outdim):
        super(Generator, self).__init__()
        self.fc = nn.Linear(z_dim, (outdim//8) * (outdim//8) * gf_dim)
        self.conv1 = nn.ConvTranspose2d(gf_dim, gf_dim, kernel_size=5, stride=2, padding=2, output_padding=1)
        self.conv2 = nn.ConvTranspose2d(gf_dim, gf_dim, kernel_size=5, stride=2, padding=2, output_padding=1)
        self.conv3 = nn.ConvTranspose2d(gf_dim, c_dim, kernel_size=5, stride=2, padding=2, output_padding=1)
    ...
    def forward(self, x):
        out = self.lrelu(self.fc(x))
        out = out.reshape(-1, self.gf_dim, self.outdim//8, self.outdim//8)
...
class Discriminator(nn.Module):
    def __init__(self, c_dim, df_dim, outdim):
        super(Discriminator, self).__init__()
        self.conv1 = nn.Conv2d(c_dim, df_dim, kernel_size=5, stride=2)
        self.conv2 = nn.Conv2d(df_dim, df_dim, kernel_size=5, stride=2)
        self.conv3 = nn.Conv2d(df_dim, df_dim, kernel_size=5, stride=2)
        self.fc1 = nn.Linear((outdim//8) * (outdim//8) * df_dim , 512)
    ...
    def forward(self, x):
        ...
        out = out.reshape(-1, (self.outdim//8) * (self.outdim//8) * self.df_dim)
